...
export default class InterfaceScene extends BaseScene {
    ...
    preload() {
        ...
        this.load.spritesheet("things2bis", "assets/things2.png", {
            frameWidth: 16,
            frameHeight: 16,
        });
    ...
    create() {
        ...
        this.input.keyboard.on("keydown_I", (event) => {
            if (
                this.invGraphics.visible ||
                document.activeElement === chatInput
            )
                this.invGraphics.visible = false;
            else this.invGraphics.visible = true;
        });
        ...
        globalEvents.on(
            ...
            (nb) => this.handleTransactionsChange(nb),
            this
        );
        ...
        globalEvents.on(
            "real-transaction",
            (value) => this.handleRealChange(value),
            this
        );
        globalEvents.on(
            "usdc-transaction",
            (value) => this.handleUSDCChange(value),
            this
        );
        ...
        globalEvents.on(
            "says",
            (message) => createTextBox(this, message),
            this
        );
        globalEvents.on(
            "chat-says",
            (name, message) => this.updateChat(name, message),
            this
        );
    }
