    ...
    update(inputKeys, t, dt) {
        ...
        if (inputKeys.left.isDown) {
            ...
            if (this.body.speed != 0) {
                ...
                if (
                    ...
                    this.anims.currentAnim.key.substring(0, 4) !== "idle"
                ) {
                    this.anims.play(
                        this.char.name + "-idle-" + this.direction.name
                    );
                }
            }
        }
        ...
        if (Phaser.Input.Keyboard.JustDown(inputKeys.space)) {
            ...
            if (this.scene.quest === "catch transactions") {
                        ...
                        if (this.triggered == false) {
                            ...
                            if (t.isValid) {
                                ...
                                globalEvents.emit(
                                    ...
                                    this.scene.transactionsCaptured
                                );
                ...
                this.scene.physics.add.overlap(
                    this.scene.actionsGroup,
                    this.scene.BuyCoffee,
                    (a, p) => {
                        if (this.triggered == false) {
                            this.scene.sound.play("machine");
                            if (this.scene.eth) this.scene.eth.buyCoffee();
                            else
                                globalEvents.emit(
                                    "says",
                                    "You must get connected first. Go see the fox"
                                );
                            this.triggered = true;
                        }
                    },
                ...
                this.scene.physics.add.overlap(
                    ...
                    (a, p) => {
                        if (this.triggered == false) {
                            if (this.scene.eth) this.scene.eth.buyUSDC();
                            else
                                globalEvents.emit(
                                    "says",
                                    "You must get connected first. Go see the fox"
