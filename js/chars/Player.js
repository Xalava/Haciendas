        ...
        if (inputKeys.space.isDown){ // alternative JustDown, at pressing
            if (this.ongoingAction == false ){
                if(this.scene.quest === "catch transactions"){
                    ...
                    this.capture = false
                    const actionNet = this.scene.add.image(this.x+13*this.direction.x, this.y+14*this.direction.y, 'actions', 0)
                    actionNet.setDepth(25)
                    //this.physics.world.enable(actionNet)
                    this.scene.physics.add.existing(actionNet)
                    this.scene.physics.add.collider(actionNet, this.scene.transactions, (a,t)=>{
                        ...
                        if (this.capture == false){
                            ...
                            if (this.transactionsCaptured>=5){
                                this.scene.quest = "create-a-block"
                            }
                        }
                    });
                    ...
                    // Generic action
                    const actionSprite = this.scene.add.image(this.x+13*this.direction.x, this.y+14*this.direction.y)
            ...
            // TODO cast a net when catching transactions
